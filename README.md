...
Sessions add a cookie to the user's browser, and the server saves an instance of the cookie to check if it is still valid, web tokens are sent to the client but not saved. Users send that token for access.

2. What does `bcrypt` do to help us store passwords in a secure manner.
It hashes the password so it's not just plain text.

3. How are unit tests different from integration and end-to-end testing.
Unit tests test one function, end to end mimics the user experience.

4. How _Test Driven Development_ changes the way we write applications and tests.
Test Driven Dev writes the tests first and you write code just to pass the test, makes sure we have all functionality covered.
